...
function InsertSchoolPage() {
  ...
  const handleFileUpload = async (file: File, fileType: 'logo' | 'banner') => {
    ...
    // Remove the 'school-media/' prefix since it's already the bucket name
    const fileName = `${fileType}-${Date.now()}-${file.name.toLowerCase()}`;
    
    try {
      const { data, error } = await supabase.storage
        .from('school-media')
        .upload(fileName, file, {  // Now just using fileName instead of filePath
          cacheControl: '3600',
          upsert: false
        });
  ...
  return (
    <div className="container mx-auto py-10 max-w-md">
      ...
      {message && (
        <Alert variant={message.includes("successfully") ? "default" : "destructive"} className="mt-4">
...
export default InsertSchoolPage;